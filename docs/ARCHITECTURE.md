...
- `main/subagent/subagent_service.c`：后台子代理任务中心（队列、执行、持久化、回传）
...
- 事件类型：`im.message.receive_v1`（text/image/audio/file 等）与 `card.action.trigger`
...
- 消息反应：入站消息会先添加 thinking reaction；最终回复后移除。若 reaction API 不可用，自动降级为占位消息并在完成后删除
- 卡片回调：优先通过 ACK 携带 toast 快速确认，再异步入队到 Agent 处理
...
- Subagent：持久化在 `/spiffs/subagent_jobs.json`，后台 worker 串行执行
