...
{
    s_inbound_queue = xQueueCreate(LMCHAN_BUS_QUEUE_LEN, sizeof(lmchan_msg_t));
    s_outbound_queue = xQueueCreate(LMCHAN_BUS_QUEUE_LEN, sizeof(lmchan_msg_t));
    ...
    ESP_LOGI(TAG, "Message bus initialized (queue depth %d)", LMCHAN_BUS_QUEUE_LEN);
    return ESP_OK;
}
