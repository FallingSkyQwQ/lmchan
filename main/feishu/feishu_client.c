...
#define FS_THINKING_EMOJI            "thinking_face"
#define FS_THINKING_PLACEHOLDER_TEXT "\xF0\x9F\xA4\x94 thinking..."

typedef struct {
    char source_message_id[96];
    char chat_id[96];
    bool use_placeholder;
    char placeholder_message_id[96];
    int64_t updated_ms;
} thinking_state_t;

static thinking_state_t s_thinking_states[32] = {0};
static bool s_reaction_unavailable = false;
...
static esp_err_t http_request_raw(const char *url, esp_http_client_method_t method,
                                  const char *auth_bearer,
                                  const char *content_type,
                                  const uint8_t *body, size_t body_len,
                                  char **out_resp)
{
    if (!url || !out_resp) return ESP_ERR_INVALID_ARG;
    *out_resp = NULL;
...
static thinking_state_t *thinking_state_find(const char *source_message_id)
{
    if (!source_message_id || !source_message_id[0]) return NULL;
    for (size_t i = 0; i < sizeof(s_thinking_states) / sizeof(s_thinking_states[0]); i++) {
        if (strcmp(s_thinking_states[i].source_message_id, source_message_id) == 0) {
            return &s_thinking_states[i];
        }
    }
    return NULL;
}

static thinking_state_t *thinking_state_get_or_alloc(const char *source_message_id, const char *chat_id)
{
    thinking_state_t *slot = thinking_state_find(source_message_id);
    if (slot) return slot;

    size_t oldest = 0;
    int64_t oldest_ms = INT64_MAX;
    for (size_t i = 0; i < sizeof(s_thinking_states) / sizeof(s_thinking_states[0]); i++) {
        if (!s_thinking_states[i].source_message_id[0]) {
            oldest = i;
            oldest_ms = -1;
            break;
        }
        if (s_thinking_states[i].updated_ms < oldest_ms) {
            oldest_ms = s_thinking_states[i].updated_ms;
            oldest = i;
        }
    }

    slot = &s_thinking_states[oldest];
    memset(slot, 0, sizeof(*slot));
    strncpy(slot->source_message_id, source_message_id, sizeof(slot->source_message_id) - 1);
    if (chat_id) {
...
static void thinking_state_clear(const char *source_message_id)
{
    thinking_state_t *slot = thinking_state_find(source_message_id);
    if (slot) {
        memset(slot, 0, sizeof(*slot));
...
static char *json_print_or_null(cJSON *obj)
{
    if (!obj) return NULL;
    return cJSON_PrintUnformatted(obj);
}

static void push_inbound_event(const char *chat_id,
                               ...
                               cJSON *raw_obj)
{
    ...
    if (source_message_id) {
        strncpy(msg.source_message_id, source_message_id, sizeof(msg.source_message_id) - 1);
    }
    if (msg_type) {
        strncpy(msg.message_type, msg_type, sizeof(msg.message_type) - 1);
    }
    if (event_type) {
        strncpy(msg.event_type, event_type, sizeof(msg.event_type) - 1);
    }
    msg.content = strdup(text);
    msg.raw_json = json_print_or_null(raw_obj);
    if (!msg.content) return;
    if (msg.source_message_id[0]) {
        feishu_begin_thinking(msg.source_message_id, chat_id);
    }
    if (message_bus_push_inbound(&msg) != ESP_OK) {
        ESP_LOGW(TAG, "Inbound queue full, drop feishu message");
        if (msg.source_message_id[0]) {
            feishu_finish_thinking(msg.source_message_id);
        }
        free(msg.content);
        free(msg.raw_json);
    }
...
static void handle_feishu_event_payload(cJSON *payload)
{
    ...
    if (!event_type) {
        event_type = cJSON_GetStringValue(cJSON_GetObjectItem(event, "event_type"));
    }
    ...
    if (strcmp(event_type, "card.action.trigger") == 0) {
        const char *chat_id = cJSON_GetStringValue(cJSON_GetObjectItem(event, "open_chat_id"));
        if (!chat_id) {
            chat_id = cJSON_GetStringValue(cJSON_GetObjectItem(event, "chat_id"));
        }
        const char *source_message_id = cJSON_GetStringValue(cJSON_GetObjectItem(event, "open_message_id"));
        cJSON *action = cJSON_GetObjectItem(event, "action");
        char action_buf[768] = {0};
        char *action_json = action ? cJSON_PrintUnformatted(action) : NULL;
        snprintf(action_buf, sizeof(action_buf), "[feishu/card_action] %s", action_json ? action_json : "{}");
        free(action_json);
        if (chat_id && chat_id[0]) {
            push_inbound_event(chat_id, source_message_id, "card.action.trigger", "card_action", action_buf, event);
            feishu_send_message(chat_id, "已收到卡片操作，处理中...");
        }
        return;
    }
    ...
    if (!chat_id || !msg_type || !content_raw) {
        return;
    }
    ...
    if (strcmp(msg_type, "text") == 0) {
        text = cJSON_GetStringValue(cJSON_GetObjectItem(content_json, "text"));
        if (!text || !text[0]) {
        ...
        if (!should_forward_group_message(message, NULL)) {
            cJSON_Delete(content_json);
            return;
        }
        const char *image_key = cJSON_GetStringValue(cJSON_GetObjectItem(content_json, "image_key"));
        snprintf(text_buf, sizeof(text_buf), "[feishu/image] image_key=%s", image_key ? image_key : "(none)");
    } else if (strcmp(msg_type, "audio") == 0) {
        if (!should_forward_group_message(message, NULL)) {
            cJSON_Delete(content_json);
            return;
        }
        const char *file_key = cJSON_GetStringValue(cJSON_GetObjectItem(content_json, "file_key"));
        snprintf(text_buf, sizeof(text_buf), "[feishu/audio] file_key=%s", file_key ? file_key : "(none)");
        ...
        const char *file_key = cJSON_GetStringValue(cJSON_GetObjectItem(content_json, "file_key"));
        const char *file_name = cJSON_GetStringValue(cJSON_GetObjectItem(content_json, "file_name"));
        snprintf(text_buf, sizeof(text_buf), "[feishu/file] name=%s file_key=%s",
                 file_name ? file_name : "(none)", file_key ? file_key : "(none)");
    } else {
        if (!should_forward_group_message(message, NULL)) {
            cJSON_Delete(content_json);
            return;
        }
        char *raw = cJSON_PrintUnformatted(content_json);
        snprintf(text_buf, sizeof(text_buf), "[feishu/%s] %s", msg_type, raw ? raw : "{}");
        free(raw);
    }
    text = text_buf;
    ...
    ESP_LOGI(TAG, "Feishu message: type=%s chat=%s %.48s", msg_type, chat_id, text);
    push_inbound_event(chat_id, message_id, "im.message.receive_v1", msg_type, text, message);
    cJSON_Delete(content_json);
}
...
static bool feishu_send_ack_frame(const ws_pb_frame_t *in_frame, int code, int64_t biz_rt_ms,
                                  const char *custom_payload_json)
{
    if (!s_ws || !in_frame) return false;

    char payload[64];
    const char *payload_ptr = custom_payload_json;
    int pn = 0;
    if (!payload_ptr || !payload_ptr[0]) {
        pn = snprintf(payload, sizeof(payload), "{\"code\":%d}", code);
        if (pn <= 0 || pn >= (int)sizeof(payload)) return false;
        payload_ptr = payload;
    } else {
        pn = (int)strlen(payload_ptr);
    }
    ...
    if (!pb_write_bytes_field(out, sizeof(out), &pos, 8, (const uint8_t *)payload_ptr, (size_t)pn)) return false;
    ...
    if (offset + len >= total) {
        ...
        if (pb_parse_frame((const uint8_t *)s_ws_payload_buf, total, &frame)) {
            ...
            if (is_data_frame) {
                ...
                const char *ack_payload = NULL;
                if (type && strcmp(type, "card.action.trigger") == 0) {
                    ack_payload = "{\"code\":200,\"toast\":{\"type\":\"info\",\"content\":\"处理中...\"}}";
                }
                acked = feishu_send_ack_frame(&frame, 200, rt_ms, ack_payload);
...
static esp_err_t feishu_send_message_ex_internal(const char *chat_id,
                                                 const char *msg_type,
                                                 const char *content_json,
                                                 char *out_message_id,
                                                 size_t out_message_id_size)
{
    if (out_message_id && out_message_id_size > 0) {
        out_message_id[0] = '\0';
    }
    if (!chat_id || !chat_id[0] || !msg_type || !msg_type[0] || !content_json) {
    ...
    if (out_message_id && out_message_id_size > 0) {
        cJSON *data = cJSON_GetObjectItem(root, "data");
        cJSON *message_id = data ? cJSON_GetObjectItem(data, "message_id") : NULL;
        if (message_id && cJSON_IsString(message_id) && message_id->valuestring) {
            strncpy(out_message_id, message_id->valuestring, out_message_id_size - 1);
            out_message_id[out_message_id_size - 1] = '\0';
        }
    }
    cJSON_Delete(root);
...
{
    return feishu_send_message_ex_internal(chat_id, msg_type, content_json, NULL, 0);
}
...
esp_err_t feishu_send_message(const char *chat_id, const char *text)
{
    ...
    while (off < total) {
        ...
        esp_err_t err = feishu_send_message_ex_internal(chat_id, "text", content_str, NULL, 0);
        free(content_str);
        if (err != ESP_OK) {
            return err;
        }
...
{
    if (!message_id || !message_id[0]) return ESP_ERR_INVALID_ARG;
    esp_err_t err = ensure_tenant_token();
    if (err != ESP_OK) return err;

    char url[256];
    snprintf(url, sizeof(url), "https://open.feishu.cn/open-apis/im/v1/messages/%s", message_id);
    char *resp = NULL;
    err = http_request_raw(url, HTTP_METHOD_DELETE, s_tenant_token, NULL, NULL, 0, &resp);
    if (resp) free(resp);
    return err;
}
...
{
    if (!message_id || !message_id[0] || !emoji_type || !emoji_type[0]) return ESP_ERR_INVALID_ARG;
    esp_err_t err = ensure_tenant_token();
    if (err != ESP_OK) return err;

    char url[256];
    snprintf(url, sizeof(url), "https://open.feishu.cn/open-apis/im/v1/messages/%s/reactions", message_id);
    cJSON *body = cJSON_CreateObject();
    cJSON_AddStringToObject(body, "reaction_type", emoji_type);
    char *body_str = cJSON_PrintUnformatted(body);
    cJSON_Delete(body);
    if (!body_str) return ESP_ERR_NO_MEM;

    char *resp = NULL;
    err = http_post_json(url, s_tenant_token, body_str, &resp);
    free(body_str);
    if (err != ESP_OK) {
        free(resp);
        return err;
...
esp_err_t feishu_reaction_remove(const char *message_id, const char *emoji_type)
{
    ...
    snprintf(url, sizeof(url),
             "https://open.feishu.cn/open-apis/im/v1/messages/%s/reactions/%s",
...
{
    if (!spiffs_path || !spiffs_path[0] || !image_key || key_size == 0) {
        return ESP_ERR_INVALID_ARG;
    }
    image_key[0] = '\0';
    ...
    uint8_t *file_buf = calloc(1, (size_t)fsize);
    if (!file_buf) {
        fclose(f);
        return ESP_ERR_NO_MEM;
    }
    size_t n = fread(file_buf, 1, (size_t)fsize, f);
    fclose(f);
    if (n != (size_t)fsize) {
        free(file_buf);
        return ESP_FAIL;
    ...
    const char *boundary = "----lmchanboundary";
    char head[512];
    ...
    size_t body_len = (size_t)head_len + (size_t)fsize + (size_t)tail_len;
    ...
    memcpy(body + head_len, file_buf, (size_t)fsize);
    memcpy(body + head_len + fsize, tail, (size_t)tail_len);
    free(file_buf);

    char ctype[128];
    snprintf(ctype, sizeof(ctype), "multipart/form-data; boundary=%s", boundary);
    char *resp = NULL;
    err = http_request_raw("https://open.feishu.cn/open-apis/im/v1/images",
                           HTTP_METHOD_POST, s_tenant_token, ctype,
                           body, body_len, &resp);
    free(body);
    if (err != ESP_OK || !resp) {
        free(resp);
    ...
    cJSON *code = cJSON_GetObjectItem(root, "code");
    cJSON *data = cJSON_GetObjectItem(root, "data");
    cJSON *key = data ? cJSON_GetObjectItem(data, "image_key") : NULL;
    if (!cJSON_IsNumber(code) || code->valueint != 0 || !key || !cJSON_IsString(key)) {
        cJSON_Delete(root);
        return ESP_FAIL;
    }
    strncpy(image_key, key->valuestring, key_size - 1);
    image_key[key_size - 1] = '\0';
    cJSON_Delete(root);
    return ESP_OK;
}
...
{
    if (!spiffs_path || !spiffs_path[0] || !file_key || key_size == 0) {
        return ESP_ERR_INVALID_ARG;
    }
    if (!file_type || !file_type[0]) file_type = "stream";
    file_key[0] = '\0';
    ...
    const char *boundary = "----lmchanboundary";
    const char *name = strrchr(spiffs_path, '/');
    name = name ? (name + 1) : spiffs_path;
    ...
    int head_len = snprintf(head, sizeof(head),
                            ...
                            "Content-Type: application/octet-stream\r\n\r\n",
                            boundary, file_type, boundary, name, boundary, name);
    char tail[64];
    int tail_len = snprintf(tail, sizeof(tail), "\r\n--%s--\r\n", boundary);
    if (head_len <= 0 || tail_len <= 0) {
        free(file_buf);
    ...
    snprintf(ctype, sizeof(ctype), "multipart/form-data; boundary=%s", boundary);
    char *resp = NULL;
    err = http_request_raw("https://open.feishu.cn/open-apis/im/v1/files",
                           HTTP_METHOD_POST, s_tenant_token, ctype,
                           body, body_len, &resp);
    free(body);
    if (err != ESP_OK || !resp) {
    ...
    cJSON *code = cJSON_GetObjectItem(root, "code");
    cJSON *data = cJSON_GetObjectItem(root, "data");
    cJSON *key = data ? cJSON_GetObjectItem(data, "file_key") : NULL;
    if (!cJSON_IsNumber(code) || code->valueint != 0 || !key || !cJSON_IsString(key)) {
        cJSON_Delete(root);
        return ESP_FAIL;
    }
    strncpy(file_key, key->valuestring, key_size - 1);
    file_key[key_size - 1] = '\0';
    cJSON_Delete(root);
    return ESP_OK;
}
...
{
    if (!source_message_id || !source_message_id[0]) return;

    thinking_state_t *st = thinking_state_get_or_alloc(source_message_id, chat_id);
    if (!st) return;
    st->updated_ms = esp_timer_get_time() / 1000;

    if (!s_reaction_unavailable) {
        if (feishu_reaction_add(source_message_id, FS_THINKING_EMOJI) == ESP_OK) {
            st->use_placeholder = false;
            return;
        }
        s_reaction_unavailable = true;
        ESP_LOGW(TAG, "Reaction add unavailable, fallback to placeholder mode");
    }

    if (chat_id && chat_id[0]) {
        char mid[96] = {0};
        cJSON *content = cJSON_CreateObject();
        cJSON_AddStringToObject(content, "text", FS_THINKING_PLACEHOLDER_TEXT);
        char *content_str = cJSON_PrintUnformatted(content);
        cJSON_Delete(content);
        if (content_str) {
            if (feishu_send_message_ex_internal(chat_id, "text", content_str, mid, sizeof(mid)) == ESP_OK) {
                st->use_placeholder = true;
                if (mid[0]) {
                    strncpy(st->placeholder_message_id, mid, sizeof(st->placeholder_message_id) - 1);
                }
            }
            free(content_str);
        }
...
{
    if (!source_message_id || !source_message_id[0]) return;
    thinking_state_t *st = thinking_state_find(source_message_id);
    if (!st) return;

    if (st->use_placeholder) {
        if (st->placeholder_message_id[0]) {
            feishu_delete_message(st->placeholder_message_id);
        }
    } else {
        feishu_reaction_remove(source_message_id, FS_THINKING_EMOJI);
    }
    thinking_state_clear(source_message_id);
}
