...
static void outbound_dispatch_task(void *arg)
{
    ...
    while (1) {
        ...
        if (strcmp(msg.channel, LMCHAN_CHAN_FEISHU) == 0) {
            esp_err_t send_err;
            const char *msg_type = msg.message_type[0] ? msg.message_type : "text";
            if (strcmp(msg_type, "text") == 0) {
                send_err = feishu_send_message(msg.chat_id, msg.content);
            } else {
                send_err = feishu_send_message_ex(msg.chat_id, msg_type, msg.content);
            }
            if (send_err != ESP_OK) {
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(subagent_service_init());
